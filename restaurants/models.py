...
class Restaurant(models.Model):
    ...
    Ресторан.
    """
    name = models.CharField(max_length=50, unique=True, verbose_name='Название ресторана')
...
class Reserve(models.Model):
    ...
    client = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name='clients', verbose_name='Клиент')
    table = models.ForeignKey(Table, on_delete=models.SET_NULL, null=True, related_name='tables', verbose_name='Стол')
    duration = models.DurationField(default=timezone.timedelta(hours=2), verbose_name='Продолжительность')

    def __str__(self):
        return f"Бронирование {self.client} на {self.date_reserved} {self.time_reserved}"
...
class ReserveHistory(models.Model):
    ...
    """
    reservation = models.ForeignKey(Reserve, on_delete=models.CASCADE, related_name='history',
                                    verbose_name='Бронирование')
